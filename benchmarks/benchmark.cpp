...
int sc_main(int argc, char** argv)
{
    ...
    std::unique_ptr<XilinxMAO> MAO = std::make_unique<XilinxMAO>("XilinxMAO",
	    "/home/veronia/Downloads/HBM_sim/config/configuration.json");
    ...
    for(unsigned i = 0; i < BMs; i++){
        ...
        else if(benchmark == "trace"){
 
            WIDTH = MAO->memorySizeInByte;

             CPUs.emplace_back(
                std::make_unique<GenericCPU>(
                    name, CPUbuswidthInByte, CPUfreqInMHz, *term, WIDTH ));
    ...
    std::string resources = "/home/veronia/Downloads/HBM_sim/config/DRAMSys/";
